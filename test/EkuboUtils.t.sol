...
contract EkuboUtilsTest is Test {
    ...
    function testSwapAndMintWithTwoZeroXLegs() external {
        ...
        assertEq(positions.ownerOf(tokenId), owner);
        assertEq(liquidity, 7 ether);
        assertEq(amount0, 4 ether);
        assertEq(amount1, 3 ether);
        assertEq(first.balanceOf(recipient), 4 ether);
        assertEq(second.balanceOf(recipient), 3 ether);
        assertEq(source.balanceOf(address(utils)), 0);
        assertEq(first.balanceOf(address(utils)), 0);
        assertEq(second.balanceOf(address(utils)), 0);
    }

    function testSingleLegMintUsesOnlyItsOwnSlippageMin() external {
        // Guards the leg-parameter wiring: when the source is a pool token, the opposite leg's
        // amountOutMin must be irrelevant to the executed leg.
        (IERC20 first, IERC20 second) = _tokens();
        vm.startPrank(owner);
        first.approve(address(utils), 20 ether);
        ...
        (uint256 tokenId,,,) = utils.swapAndMint(params);
        vm.stopPrank();
    ...
    function testSingleLegMintFromToken1SideIgnoresToken0Min() external {
        // Mirror of testSingleLegMintUsesOnlyItsOwnSlippageMin for the token1-source branch.
        (IERC20 first, IERC20 second) = _tokens();
        vm.startPrank(owner);
    ...
    function testSwapAndMintRejectsSideOutputFromFirstLeg() external {
        // Regression for EKUB-10: with a third source token, a first-leg plan that also
        // delivers the second pool token must revert instead of stranding it unaccounted.
        MockERC20 source = new MockERC20("Source", "SRC");
        (IERC20 first, IERC20 second) = _tokens();
        source.mint(owner, 20 ether);
        vm.startPrank(owner);
